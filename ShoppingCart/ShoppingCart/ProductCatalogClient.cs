...
namespace ShoppingCart.ShoppingCart
{
    ...
    {
        public Task<IEnumerable<ShoppingCartItem>> GetShoppingCartItems(int[] productCatlogIds);
    }
    ...
    public class ProductCatalogClient : IProductCatalogClient
    {
        ...
        public async Task<IEnumerable<ShoppingCartItem>> GetShoppingCartItems(int[] productCatlogIds)
        {
            using var response =
                await RequestProductFromProductCatalog(productCatlogIds);
            return await ConvertToShoppingCartItems(response);
        }

        private async Task<HttpResponseMessage> RequestProductFromProductCatalog(int[] productCatlogIds)
        {
            var productsResource = string.Format(getProductPathTemplate,
                    string.Join(",", productCatlogIds));
